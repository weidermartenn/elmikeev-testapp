<template>
  <div class="h-full flex flex-col justify-start items-center p-4">
    ...
    <div class="p-4">
      <h1 v-if="salesStore.allSales.meta" class="text-xl text-left mb-2">
        Страница: {{ salesStore.allSales.meta.current_page }} из {{ salesStore.allSales.meta.last_page }}
      </h1>
      <span v-if="salesStore.allSales.meta" class="text-sm text-gray-400">Общее количество записей: {{ salesStore.allSales.meta.total }}</span>
      <br>
    ...
    <div class="max-h-[60vh] overflow-x-auto overflow-y-scroll p-4 rounded-sm border border-base-content/5 bg-base-100 w-full max-w-6xl">
      ...
      <table v-else class="table table-xs sm:table-sm md:table-md">
        ...
        <tbody>
          <tr v-for="(item, index) in filteredItems" :key="`${item.sale_id}-${index}`">
            <td>{{ index + 1 }}</td>
            <td>{{ item.sale_id }}</td>
            <td>
            ...
            <td>
              ...
              <span class="text-xs text-gray-400">
                {{ item.country_name }}, {{ item.oblast_okrug_name }}, {{ item.region_name }}
              </span>
            </td>
            <td>
    ...
    <div v-if="salesStore.allSales.meta" class="join grid grid-cols-2 mt-4">
      <button
        @click="prevPage"
    ...
    <div class="mt-4 mb-10 overflow-x-auto overflow-y-scroll p-4 rounded-sm border border-base-content/5 bg-base-100 w-full max-w-6xl">
      ...
      <SalesChart :sales-data="filteredItems" defer/>
    </div>
  </div>
</template>
...
import { useSalesStore } from "../model/store";
...
const uniqueWarehouses = computed(() => {
  if (!salesStore.allSales.data) return [];
  const warehouses = new Set(salesStore.allSales.data.map(item => item.warehouse_name));
  return Array.from(warehouses).sort();
});

const filteredItems = computed(() => {
  let items = salesStore.allSales.data || [];
  
  if (form.value.search) {
    const searchTerm = form.value.search.toLowerCase();
    items = items.filter(item => 
      item.supplier_article.toLowerCase().includes(searchTerm) ||
      item.sale_id.toLowerCase().includes(searchTerm)
    );
  }
...
const disablePrevPage = computed(() => {
  return (
    !salesStore.allSales.meta ||
    salesStore.allSales.meta.current_page === 1
  );
});

const prevPage = async () => {
  if (disablePrevPage.value) return;
  form.value.page = salesStore.allSales.meta.current_page - 1;
  await fetchData();
};
...
const disableNextPage = computed(() => {
  return (
    !salesStore.allSales.meta ||
    salesStore.allSales.meta.current_page === salesStore.allSales.meta.last_page
  );
});
